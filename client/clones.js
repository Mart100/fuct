$(() => {
  ...
  $('body').on('keydown', (event) => { if(event.key == 'c' && !inMenu && !isChatting()) $('#cloneControl').fadeIn(100) })
  $('body').on('keyup', (event) => { if(event.key == 'c' && !inMenu && !isChatting()) $('#cloneControl').fadeOut(100) })
